...
impl Database for PostgresDBConfig {
    ...
    async fn new(db_config: PostgresDBConfig) -> PmtreeResult<Self> {
        {
            ...
            let tx = client.build_transaction()
                ...
                .start()
                .await.expect("Database::new build_transaction.start() error");
    ...
    async fn get_pre_image(&self, key: DBKey) -> PmtreeResult<Option<Self::PreImage>> {
        ...
        let query = format!(
            ...
            pre_image=self.pre_image_table, merkle=self.merkle_table
        );
        ...
        match rows.len() {
            ...
            0 => Ok(None),
            _ => Err(DatabaseError(DatabaseErrorKind::CustomError("Primary key should be unique".to_string())))
        }
    }
    ...
    async fn put_with_pre_image(&mut self, key: DBKey, value: Value, pre_image: Option<Self::PreImage>) -> PmtreeResult<()> {
        match pre_image {
            Some(t) => {
                ...
                let owner = PgNumeric::new(Some(BigDecimal::new(BigInt::from_bytes_be(num_bigint::Sign::Plus, &t.address),0)));
                let coin_low = PgNumeric::new(Some(BigDecimal::new(BigInt::from_bytes_be(num_bigint::Sign::Plus, &t.low_coin.to_be_bytes()),0)));
                let coin_high = PgNumeric::new(Some(BigDecimal::new(BigInt::from_bytes_be(num_bigint::Sign::Plus, &t.high_coin.to_be_bytes()),0)));
                ...
                let tx = client.build_transaction()
                    .isolation_level(IsolationLevel::Serializable)
                    .start()
                    .await.expect("Database::put_with_pre_image build_transaction error");
                ...
                let rows_modified = tx.execute(&statement, &[&leaf, &owner, &coin_low, &coin_high]).await.unwrap();
                assert_eq!(rows_modified, 1, "should be only 1 new row");
                tx.commit().await.unwrap();
                Ok(())
            }
            None => Self::put(&mut self, key, value).await
        }
    }
